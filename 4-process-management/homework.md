...
В программе происходит замена исходного кода процесса без создания нового с помощью `execve`. В него мы передаём название программы, которое мы передали через параметры командной строки, также передаём агрументы командной строки для этой программы и переменные окружения. Эта новая программа выводит параметры командной строки и переменные окружения.
...
act.sa_flags = 0;
act.sa_handler = SIG_IGN; // changing to ignore flag
```
