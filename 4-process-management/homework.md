...
Когда мы запустили `ps -l` без `exec`, то он запустился как дочерний процесс текущей оболочки с новом PID, а когда мы запустили с `exec`, то исходный код процесса оболочки заменился на исходный код команды `ls`, которая после выполнения сразу отмерла.
